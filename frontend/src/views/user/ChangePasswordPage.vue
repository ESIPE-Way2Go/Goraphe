<template>
  <div class="d-flex align-center fill-height bg-image">
    <v-card class="mx-auto pa-6 bg-white rounded-xl" max-height="525px" style="border-color: blue" variant="outlined"
            width="500px">
      ...
      <v-card-text>
        <v-form v-model="isFormValid" fast-fail @submit.prevent="updateAccount">
          ...
          <v-text-field v-model="confirmPassword" :append-icon="show2 ? 'mdi-eye' : 'mdi-eye-off'"
                        :rules="confirmPasswordRules" :type="show2 ? 'text' : 'password'" class="ma-2"
                        label="Confirmation du mot de passe" variant="outlined"
                        @click:append="show2 = !show2"></v-text-field>
          <v-btn :disabled="!isFormValid" class="mt-2 rounded-3 bg-blue-darken-1 mx-auto d-table" size="large"
                 type="submit">
            Valider
          </v-btn>
        </v-form>
